...
public class Solution {
    ...
    public static long maxDuffelBagValue(CakeType[] cakeTypes, int weightCapacity) {
        long[] maxValue = new long[weightCapacity + 1];
        int weightOfBag = 0;
        long maxValueUsingCake = 0L;

        // calculate the maximum value that we can carry
       for (int currentWeight = 0; currentWeight <= weightCapacity; currentWeight++){
           long currentMaxValue = 0L;
           for (CakeType cakeType : cakeTypes){
                if (cakeType.weight == 0 && cakeType.value != 0){
                    throw new InfinityException();
                }
                if (cakeType.weight <= currentWeight){
                    maxValueUsingCake = cakeType.value + maxValue[currentWeight - cakeType.weight];
                    currentMaxValue = Math.max(maxValueUsingCake, currentMaxValue);
                }
           }
             maxValue[currentWeight] = currentMaxValue;
       }
